...
G4Material* lit_mat = nist->FindOrBuildMaterial("G4_LITHIUM_OXIDE");
G4String name;
G4int ncomponents;
G4double fracMass;
G4Material* Lidoped = new G4Material(name = "Dope_core", 1.15*g/cm3, ncomponents = 2);
Lidoped->AddMaterial(scin_mat, fracMass = 90 * perCent);
Lidoped->AddMaterial(lit_mat, fracMass = 10 * perCent);
...
Lidoped->SetMaterialPropertiesTable(fMPTPStyrene);
...
for (int i = 1; i < 21; i++){  
for (int j = 1; j < 35; j++){
                       new G4PVPlacement(rotationMatrix,G4ThreeVector(0*cm, (-1.36+(0.136*(i-1)))*cm,(-2.38+(0.136*j))*cm), XCover1,"TopLidopedXCover1", logicWorld, false, j+(i-1)*34, checkOverlaps);          //                            
  }  
  }
